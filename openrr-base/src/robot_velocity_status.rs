...
impl RobotVelocityStatus {
    pub fn new(max_velocity: BaseVelocity, max_acceleration: BaseAcceleration) -> Self {
        let min_velocity = max_velocity * (-1.);
        let min_acceleration = max_acceleration * (-1.);
    ...
    pub fn limited_velocity(&self, velocity: &BaseVelocity) -> BaseVelocity {
        let vel = self.acceleration_limiter(velocity);
        self.velocity_limiter(&vel)
    }
    ...
    fn acceleration_limiter(&self, velocity: &BaseVelocity) -> BaseVelocity {
        let delta = self.time_since_last_sent();
        let vel = self.velocity.lock().to_owned();
        BaseVelocity {
            x: velocity.x.clamp(
                vel.x + self.min_acceleration.x * delta,
                vel.x + self.max_acceleration.x * delta,
            ),
            y: velocity.y.clamp(
                vel.y - self.min_acceleration.y * delta,
                vel.y + self.max_acceleration.y * delta,
            ),
            theta: velocity.theta.clamp(
                vel.theta - self.min_acceleration.theta * delta,
                vel.theta + self.max_acceleration.theta * delta,
            ),
        }
    }
}
...
mod test {
    ...
    fn test_robot_velocity_status() {
        ...
        let limited = status.limited_velocity(&cmd_vel);
